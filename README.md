...
Filters low counts (CPM ≥ cpm_min in ≥ cpm_min_samples; plus filterByExpr).

TMM normalization and voom.

Exports: filtered raw counts, CPM (TMM), voom logCPM.

02_de_limma.R

Builds design (PBS/Lo/Med/Hi), computes contrasts vs PBS.

Fits lmFit + eBayes; writes full DE tables (.tsv) for each contrast.

03_go_enrichment.R

Ranks genes by logFC, maps IDs to Entrez, performs GSEA (GO:BP).

Saves dotplots and CSVs per contrast; robust to small gene lists.

04_plots_and_goi.R

Data-driven GOI ranking combining DE and GSEA signals.

Produces: GOI violin plots, heatmap (row-Z), bar+jitter with mean±SEM and Dunnett stars vs PBS.

Writes GOI_ranked_all.csv and two shortlists (broad/strict).

05_qc_and_volcano.R

QC: library sizes, MDS, PCA, MA plots;

Volcano plots per contrast (with optional labels).

Dynamic handling of available contrast names.

06_go_networks.R

Term–term similarity networks (emapplot) and gene–term networks (cnetplot).

Uses fold-change colors where available; saves PNG/PDF.

07_goi_zoom.R

Focused analysis for specified GOIs (default: Bub1, Cenpi, Esco2, Rpl36-ps12).

Plots violin / bar+jitter for these genes;

Venn diagrams for leading-edge genes (global) and for Lo vs PBS;

Exports per-gene summary tables.
